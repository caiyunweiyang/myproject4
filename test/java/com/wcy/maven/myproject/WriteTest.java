package com.wcy.maven.myproject;
...
import com.wcy.maven.myproject.handle.handler.CommentWriteHandler;
import com.wcy.maven.myproject.handle.handler.CustomCellWriteHandler;
import com.wcy.maven.myproject.handle.handler.CustomSheetWriteHandler;
import com.wcy.maven.myproject.pojo.*;
import com.wcy.maven.myproject.pojo.head.ComplexHeadData;
import com.wcy.maven.myproject.util.DateUtil;
import com.wcy.maven.myproject.util.TestFileUtil;
...
public class WriteTest {
    ...
    public void customHandlerWrite() {
        String basePath = "F:/data1/outbound/dev/etc/upload/";
        String yearMonth = DateUtil.format(new Date(), DateUtil.YEAR_MONTH_FORMAT_STR);
        File fileDir = new File(basePath + yearMonth);//根据地址去找文件夹，若无文件夹则创建一个文件夹，避免文件夹写死和找不到文件夹
        if (!fileDir.exists()) {
            fileDir.mkdirs();
            try {
                Runtime.getRuntime().exec("chmod 777 -R " + fileDir);
            } catch (IOException e) {
                log.error("export merchant transaction error:{}", e);
            }
        }
        ...
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(filePath.toString(), DemoData.class).registerWriteHandler(new CustomSheetWriteHandler())
    ...
    public void modifyHead() {
        //指定文件导出到指定路径
        String basePath = "F:/data1/outbound/dev/etc/upload/";
        String format = DateUtil.format(new Date(), DateUtil.YEAR_MONTH_FORMAT_STR);
        ...
        if (!file.exists()){
            ...
            try {
                ...
                log.error("exprot excel error:{}",e);
            }
        }
        StringBuilder filePath=new StringBuilder()
                .append(basePath)
                ...
                .append(System.currentTimeMillis())
                .append(".xlsx");
       // String fileName = TestFileUtil.getPath()+"啦啦啦" +System.currentTimeMillis() + ".xlsx";
        // 这里 需要指定写用哪个class去写，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        // 如果这里想使用03 则 传入excelType参数即可
        EasyExcel
                .write(filePath.toString(), DemoData.class)
                .sheet("导出自定义表头文件")
